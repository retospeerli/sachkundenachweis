    ...
    function buildImageLibrary(fileNames) {
      ...
      for (const item of IMAGE_ITEMS) {
        const aliases = (item.aliases || [item.key]).map(normaliseImageName);
        library[item.key] = fileNames.filter(fileName =>
          IMAGE_EXTENSIONS.test(fileName)
          && aliases.some(alias => normaliseImageName(fileName).includes(alias))
        );
      }
      return library;
    }
    ...
    function questionHasImage(question) {
      return !question.imageKey || (imageLibrary[question.imageKey] || []).length > 0;
    }

    function imageQuestionText(group) {
      if (group === "fish") return "Bestimme diesen Fisch.";
      if (group === "rod") return "Was ist das für eine Rute?";
      if (group === "reel") return "Welche Rolle ist hier abgebildet?";
      return "Welcher künstliche Köder ist hier abgebildet?";
    }

    function createSupplementalImageQuestion(item) {
      const choices = shuffle([
        item,
        ...shuffle(IMAGE_ITEMS.filter(candidate => candidate.group === item.group && candidate.key !== item.key)).slice(0, 2)
      ]);
      const isFish = item.group === "fish";
      return {
        id: 10000 + IMAGE_ITEMS.indexOf(item),
        question: imageQuestionText(item.group),
        options: choices.map(choice => choice.label),
        answer: choices.findIndex(choice => choice.key === item.key),
        category: isFish ? "fish" : "gear",
        categoryTitle: isFish ? "Fischkunde" : "Methoden & Geräte",
        topic: isFish ? "identify" : "methods",
        topicTitle: isFish ? "Fische erkennen" : "Methoden & Geräte",
        image: null,
        imageKey: item.key,
        supplemental: true
      };
    ...
    function supplementalQuestions(topicSlug) {
      if (topicSlug !== "identify" && topicSlug !== "methods") return [];
      const represented = new Set(
        QUESTIONS.filter(question => question.topic === topicSlug && question.imageKey)
          .map(question => question.imageKey)
      );
      return IMAGE_ITEMS
        .filter(item => topicSlug === "identify" ? item.group === "fish" : item.group !== "fish")
        .filter(item => !represented.has(item.key) && (imageLibrary[item.key] || []).length > 0)
        .map(createSupplementalImageQuestion);
    }

    function topicCount(topic) {
      return QUESTIONS.filter(question => question.topic === topic.slug && questionHasImage(question)).length
        + supplementalQuestions(topic.slug).length;
    }
      ...
      app.innerHTML = `
          ...
          <div class="topic-grid">
            ${TOPICS.map(topic => {
              ...
              return `<button class="topic" type="button" data-action="start-learn" data-topic="${topic.slug}">
                ...
                <small>${topicCount(topic)} Fragen</small>
        ...
        <aside class="source">
          ...
          <div><strong>Fragenbasis und echter Prüfungsmodus</strong>
            ...
            <p><strong>Erweiterbare Bildsammlung:</strong> Fisch-, Köder-, Ruten- und Rollenbilder im Ordner <code>images</code> werden auf GitHub Pages automatisch erkannt. Der Dateiname enthält die Art, beispielsweise <code>wels1.jpg</code>, <code>hecht2.png</code>, <code>spinner3.webp</code> oder <code>multirolle1.jpg</code>. Bildfragen ohne tatsächlich vorhandene passende Datei werden automatisch entfernt.</p>
          </div>
        </aside>`;
      window.scrollTo({ top: 0, behavior: "smooth" });
    }

    async function startLearning(topicSlug) {
      const topic = TOPICS.find(item => item.slug === topicSlug);
      if (!topic) return;
      if (imageDiscoveryPromise) await imageDiscoveryPromise;
      const baseQuestions = QUESTIONS.filter(question => question.topic === topicSlug && questionHasImage(question));
      const session = shuffle([...baseQuestions, ...supplementalQuestions(topicSlug)]);
      if (!session.length) {
        app.innerHTML = `<section class="result-card empty"><h2>Keine passenden Bilder gefunden</h2><p>Füge korrekt benannte Dateien in den Ordner <code>images</code> ein und lade die Seite neu.</p><button class="btn btn-primary" type="button" data-action="home">Zur Startseite</button></section>`;
        return;
      }
      state = {
        screen: "quiz",
    ...
    function startExam() {
      const availableQuestions = QUESTIONS.filter(question => questionHasImage(question));
      state = {
        screen: "quiz",
        mode: "exam",
        session: shuffle(availableQuestions).slice(0, 50),
        index: 0,
        answers: {},
        revealed: false,
        title: "Prüfungsmodus",
    ...
    function renderQuiz() {
      ...
      if (question.imageKey && !selectedImage) {
        state.session.splice(state.index, 1);
        if (state.index >= state.session.length) state.index = Math.max(0, state.session.length - 1);
        renderQuiz();
        return;
      }
    ...
    app.addEventListener("click", async event => {
      ...
      else if (action === "start-learn") await startLearning(target.dataset.topic);
    ...
    renderHome();
    imageDiscoveryPromise = discoverImageFiles();
